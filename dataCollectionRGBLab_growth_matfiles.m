function[data] =dataCollectionRGBLab_growth_matfiles(filename)
...
for k = 1:length(fnames)
    ...
    if k == 1
        ...
        idAll = s.(StrName).ID;
        centroidsAll = s.(StrName).centroids;
        areaAll = s.(StrName).area;
        diametert1All = s.(StrName).diameter_t1;
       ...
       Labt_meanAll = s.(StrName).Labt_mean;
        ...
        centroidsAll = [centroidsAll; s.(StrName).centroids];
        areaAll = [areaAll; s.(StrName).area];
        diametert1All = [diametert1All; s.(StrName).diameter_t1];
