...
%Extract centroids
centroid = Tfilter.centroid;
%Extract diameters
diameter = Tfilter.diameter;
...
%     hold on
    ...
    for i = 1:length(temp_cluster_mean) 
        ...
        figure
        imshow(B);
        print(strcat('c',string(temp_cluster_mean(i)),'_',output{1}, '-', string(i)), '-dpng');
        close;       
    end 
    %hold off
...
% 
% %Save as .mat
Tfilter.sumActivity = sum(table2array(Tfilter(:,27:39)), 2);
data_isolationFilter = table2struct(Tfilter);
save('pixel_bio_filter-Data.mat', 'data_isolationFilter');
